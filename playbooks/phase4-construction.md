...
- `aidlc-docs/audit.md` に対話サマリーを記録する
...
4. **インデックス戦略を定義する**
   - 外部キー、検索対象カラムにインデックスを設計する
   - クエリパターンに基づき必要なインデックスを明記する
...
- `aidlc-docs/audit.md` に実装サマリーを記録する
- テスト結果、動作確認の結果を記録する

### 完了条件

- 全機能が実装された
- テストコードが書かれている（最低限 API エンドポイントの統合テストを含むこと）
- ビルド成功 + 全テストがパスしている
- アプリケーションが動作する状態になった
